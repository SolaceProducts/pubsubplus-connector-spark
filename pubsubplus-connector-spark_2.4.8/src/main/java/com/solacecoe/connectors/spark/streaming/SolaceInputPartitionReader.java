...
public class SolaceInputPartitionReader implements InputPartitionReader<InternalRow>, Serializable {
...
//    }
    ...
    public SolaceInputPartitionReader(List<SolaceRecord> values, boolean includeHeaders) {
        this.values = values;
        this.includeHeaders = includeHeaders;
    }
    ...
    @Override
    public boolean next() {
        if(!this.values.isEmpty() && index < this.values.size()) {
            solaceRecord = this.values.get(index);
...
//            } else {
                ...
                log.info("SolaceSparkConnector - Total messages in InputPartition " + this.values.size());
                return true;
//            }
        }
