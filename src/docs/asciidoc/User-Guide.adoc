...
NOTE: Partitioned Queue's is not fully supported. The connector can process data from partition queue but checkpointing per partition key is not yet supported.
