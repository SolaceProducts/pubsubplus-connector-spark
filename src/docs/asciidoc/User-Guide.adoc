...
The connector supports multiple checkpoint storage options depending on the deployment platform. When running on Databricks, Unity Catalog Volumes can be used as the checkpoint location. Since executor nodes do not have direct access to Unity Catalog Volumes, the connector uses unified authentication via a Databricks service principal to establish secure access.

To enable this, the following must be configured:

* Runtime Platform — Set to DATABRICKS to enable Unity Catalog Volume support
* Secret Scope — A Databricks secret scope containing the service principal credentials (databricks_host, databricks_client_id, databricks_client_secret)
* Secret Refresh Interval — Controls how frequently the connector fetches updated credentials from the secret scope to handle secret rotation without requiring a restart

IMPORTANT: Both secret rotation and the connector's credential refresh must complete before the secret expires. The refresh interval must be configured to trigger after rotation completes and before the existing secret expires. See the configuration section for detailed guidance on configuring the refresh interval correctly.
