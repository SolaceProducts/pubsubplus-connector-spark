...
When using an All-Purpose compute cluster, the Spark session is not automatically terminated upon job or notebook cancellation, as the cluster remains active for extended periods. In such scenarios, the connection to Solace can remain idle, which may impact connection usage on the broker.
...
The Solace Spark Connector monitors the time since the last processed message. If this duration exceeds the configured idle timeout, the connector proactively closes the idle connection.

NOTE: Both configurations are disabled by default and must be explicitly set to enable this feature.
