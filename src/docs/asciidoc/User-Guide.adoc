...
The connector supports multiple checkpoint storage options depending on the deployment platform. When running on Databricks, Unity Catalog Volumes can be used as the checkpoint location. Since Databricks executor nodes do not have direct access to Unity Catalog Volumes, the connector uses https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/oauth-m2m[unified authentication] via a `Databricks workspace level service principal` to establish secure access. Ensure the Databricks workspace level service principal has access to Unity Catalog Volume configured as checkpoint storage.

To enable this, the following must be configured:

* Runtime Platform — Set to DATABRICKS to enable Unity Catalog Volume support
* Secret Scope — A Databricks secret scope containing the workspace level service principal credentials (databricks_host, databricks_client_id, databricks_client_secret)
* Client Secret Refresh Interval — Controls how frequently the connector fetches updated client secret from the secret scope to handle client secret rotation without requiring a restart
