...
| any
| empty
| Absolute path to client certificate file for TLS connections. Make sure file path is accessible by the connector.
...
2. If solace.oauth.client.auth-server.client-certificate.file is configured simply provide a path(should include file name as well) to create a JKS file by the connector.
...
2. If solace.oauth.client.auth-server.client-certificate.file is configured simply provide a password which will be used to protect the JKS file create in above property.

| solace.oauth.client.auth-server.ssl.validate-certificate
| boolean
| any
| false
| Boolean value to enable or disable ssl certificate validation. Set this true in production environment.
...
| positive integer value
| 100
| Connection timeout(Milliseconds) for access token request.
