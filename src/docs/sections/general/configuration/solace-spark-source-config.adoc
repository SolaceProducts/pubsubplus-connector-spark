...
| string
| valid databricks client secret
| empty
...
| 1440
| Specifies how frequently the connector fetches the latest client secret from the configured Databricks secret scope. Since the connector cannot detect secret rotations automatically, this interval should be configured based on your organization's secret rotation frequency to ensure the connector always operates with valid credentials.

Configuration guidance:

Set this value slightly higher than your organization's secret rotation period. This ensures the connector fetches the new secret shortly after rotation occurs, regardless of when within the rotation period it happens.

For example, if secrets are rotated every 60 minutes, setting the refresh interval to 65 minutes ensures the connector fetches the new secret within 65 minutes of rotation — before the old secret expires or access is revoked.

WARNING: If the refresh interval is significantly longer than the secret rotation period, the connector may attempt to authenticate with an expired secret, causing access failures to Unity Catalog Volumes until the next refresh cycle.
