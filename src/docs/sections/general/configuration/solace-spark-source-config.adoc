...
| any
| 1
| Set number of messages to be processed in batch. The connector can stream data in batches to Spark based on configured size. For efficient processing by Spark set Maximum Delivered Unacknowledged Messages per Flow property on Queue is twice the batch size.
